...
| Roll20 Help Center: iFrame and Sanitization Changes | Roll20 distinguishes the legacy sanitizer from the newer iframe system and states that modern sheets retain substantially more authored HTML/CSS. | Use as the official generation boundary; keep modern and legacy preview/export contracts atomic and separately verified. | It does not document every current URL transformation surface. Direct Sandbox/test-room DOM evidence wins for image, stylesheet, inline-style, and font behavior. |
| Roll20 Wiki: Legacy Sheet Sanitization | The legacy reference documents class restrictions and Roll20 image proxy behavior. | Use as legacy sanitizer background and a hypothesis checklist for allow-list/prefix/proxy tests. | Wiki detail may be historical; validate current behavior in the dedicated legacy room before changing the renderer. |
...
- [Roll20 Help Center: iFrame and Sanitization Changes](https://help.roll20.net/hc/en-us/articles/360061735034-iFrame-and-Sanitization-Changes)
- [Roll20 Wiki: Legacy Sheet Sanitization](https://wiki.roll20.net/Legacy_Sheet_Sanitization)
