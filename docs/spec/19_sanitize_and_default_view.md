...
- 본 프로젝트의 legacy-sheet-corpus legacy corpus / modern corpus 검증에서 확인된 케이스
...
`~` (general sibling) selector 가 sandbox 에서 동작 — legacy-sheet-corpus legacy corpus / modern corpus 합치기에 쓰는 패턴.
...
legacy-sheet-corpus / legacy corpus / modern corpus / 펄프 등 시스템 specific 식별자는 **블록 정의에 들어가지 않는다**. 사용자가 ID / LABEL 을 직접 입력. block label 도 일반 한국어 ("토글 체크박스", "켜졌을 때 보이는 영역", "꺼졌을 때 보이는 영역").

### 2.4 측정 불가 — 솔직히

- `~` (general sibling) selector 가 모든 Roll20 sandbox 버전에서 동작하는지는 **측정 불가** (sandbox PoC 필요). community 보고 + legacy-sheet-corpus legacy corpus working copy 의 동작 사례 기반 추정.
