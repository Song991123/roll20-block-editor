...
legacy-sheet-corpus legacy corpus (732 KB HTML, CoC 7e 변형) import 시:
...
- **renderer freeze** — Blockly 가 1000+ 블록 동시 인스턴스화하면 main thread
  blocking. legacy-sheet-corpus 외 어떤 커스텀 시트도 동일.
...
legacy-sheet-corpus legacy corpus + 공식 시트 (`docs/spec/20_official_sheet_patterns.md`,
...
| 2 | `r20_skill_row` | **P0** | 매우 높음 | CoC 50 + legacy-sheet-corpus 100 + 인세인 / PbtA `<tr>` 행 패턴 | ~12:1 | Phase 2 |
...
legacy-sheet-corpus legacy corpus 의 능력치 카드 atomic 시퀀스 (1 행 안 td chain):
...
legacy-sheet-corpus legacy corpus 8 능력치 × atomic 7~9 = ~64 atomic. Phase 1 적용 후 → 8 composite.
**약 56 block 감소.**
...
dot tracker / pbta move (Phase 3, legacy-sheet-corpus 비해당) — legacy-sheet-corpus legacy corpus 의 9093 → ~1500
...
- composite block 의 모든 field 는 `LABEL` / `ATTR_NAME` / `ROLL_EXPR` 등
  **general parameter**. legacy-sheet-corpus / D&D 5e / PbtA / CoC 무엇이든 같은 schema.
- matcher 의 인식 조건은 HTML 구조 + 표준 Roll20 idiom (`attr_X` /
  `data-i18n="KEY"` / `type="roll"`) 만. 한글 라벨 / legacy-sheet-corpus specific class
  hardcoding 0.
...
1. **Phase 2** — `r20_skill_row` 추가. atomic 12 → composite 1 (legacy-sheet-corpus legacy corpus
   100+ 행 → ~1100 atomic 감소).
2. **Phase 2** — `r20_repeating_section_wrapper`. legacy-sheet-corpus 12 instance —
   `<fieldset class="repeating_X">` + `<thead>` + `<tbody>` packing.
3. **Phase 3** — `r20_dot_tracker` (WoD / BitD). legacy-sheet-corpus 비해당이지만 generic
   compat 강화.
...
- 본 phase 의 측정값 (`r20_attribute_card` matcher 만 적용) 은 legacy-sheet-corpus legacy corpus
...
- `r20_attribute_card` matcher 의 인식 조건이 너무 보수적이면 우연 매칭 0
  대신 legacy-sheet-corpus 8 능력치 일부도 못 잡을 수 있음. 본 phase 는 false-negative
