...
**Backward compat**: legacy-sheet-corpus legacy corpus 등 LANG 미사용 시트는 기존 emit 보존 → 매칭 회귀 0.
...
legacy-sheet-corpus era toggle 같은 "값에 따라 다른 영역 보이기" 패턴을 일반화. 시스템 specific
...
**legacy-sheet-corpus era toggle 과 호환**: era panel 5개 (`pulp` / `modern` / ...) 가 본 패턴과
...
- legacy-sheet-corpus legacy corpus 회귀: `htmlMatched=6134/6134, cssMatched=202/202, htmlRawFallback=0` — 매칭 회귀 0. (legacy-sheet-corpus legacy corpus 자체는 emit 형식의 `sheet-X-switch` / `getCompendium*` / CSS `--var` / 단일 `@{...}` token EXPR 패턴이 거의 없으므로 신규 매처 trigger 0 — 정상.)
...
- `r20_value_switch_panel` 의 inline `<style>` 가 Roll20 sandbox 에서 적용되는지 — legacy-sheet-corpus
  legacy corpus era pattern 과 등가이므로 정상 동작 추정. 다른 sandbox 정책 변경 시 영향.
