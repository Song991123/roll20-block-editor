...
| Local preview vs edit | VERIFY/GOOD_LOCAL | `smoke:preview-edit-visual` PASS on 2026-07-13: fixture-A `1.86%`, fixture-B `2.07%`, fixture-C `1.02%`. `smoke:imported-edit-sync` also passes for the prepared fixtures. | Preview/edit share enough local rendering behavior to keep improving edit UX, but exact Roll20 parity is still unproven. |
...
| Renderer promotion | BLOCKED | `gate:roll20-renderer-action`: `HOLD_PRODUCTION_RENDERER_PATCH`, `rendererBlockers=8`, `rendererReady=NO`. The gate also reports asset-relink blockers for fixture-A/fixture-C and split template scopes `.sheet-rolltemplate-aw` vs `.sheet-rolltemplate-coc`. | No production Roll20 renderer CSS/chat patch should be promoted yet. Global ChatPane tweaks are specifically unsafe. |
| Asset relink / browser paint | BLOCKED_BY_USER_ASSET_URLS | `plan:roll20-asset-relink` reports `RELINK_MAP_REQUIRED`: fixture-A and fixture-C are `MISSING_RELINK`. `plan:roll20-chat-browser-paint` reports `BROWSER_PAINT_BLOCKED_BY_RELINK`. Import/export can generate commented relink drafts, export smoke verifies the draft path, and placeholder targets are now rejected/count as `미입력` instead of being applied. The CLI relink plan also keeps placeholder maps at `MISSING_RELINK` with `mapEntries=0`. | The product can guide users toward the needed map and avoids false relink readiness, but it still needs real user-owned HTTP(S) replacement URLs before background/paint pixels can be judged or renderer CSS promoted for those fixtures. |
...
| Actual Roll20 chat/rolltemplate reproduction | 0-5% | 40-50% | Capture quality improved to authoritative `6/6` generated screenshots/diffs with `3/3` normalized chat comparisons, but fixture-A/fixture-C still mismatch visually and renderer remains held. |
...
=> rendererBlockers=8
=> asset relink blocks fixture-A/fixture-C chat background judgment
...
=> RELINK_MAP_REQUIRED
=> fixture-A MISSING_RELINK
=> fixture-C MISSING_RELINK
...
=> fixture-A BLOCKED_BY_ASSET_RELINK
=> fixture-C BLOCKED_BY_ASSET_RELINK
=> fixture-B PAINT_SECONDARY_NO_BACKGROUND_IMAGE

corepack pnpm run smoke:legacy-fixture-visual -- --port 4410
=> 95.9s
=> PASS fixture-A risk=0->0 diff=16.90%
=> PASS fixture-B risk=1->0 diff=0.04%
=> PASS fixture-c-commission-1bu risk=0->0 diff=35.73%
...
=> fixture-A NEEDS_NEW_DIAGNOSTIC_MODEL
=> fixture-C NEEDS_NARROW_TEMPLATE_MODEL
...
=> PASS fixture-A mismatch=1.86%
=> PASS fixture-B mismatch=2.07%
=> PASS fixture-c-commission-1bu mismatch=1.02%
...
| --- | ---: | --- |
| Evidence-safe MVP checkpoint | 2-4 working days | fixture-A/fixture-C trusted chat captures are recaptured or clearly replaced by a verified adapter; renderer gate blockers are reduced and documented; no false evidence path remains. |
...
1. Resolve asset relink blockers for fixture-A/fixture-C.
...
2. Build narrower chat renderer models instead of global ChatPane CSS.
   - fixture-A: `.sheet-rolltemplate-aw` message/content width plus exact text metrics.
   - fixture-C/CoC: `.sheet-rolltemplate-coc` table intrinsic, sanitize-order, and font-context model.
