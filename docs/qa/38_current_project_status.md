...
| Asset relink / browser paint | BLOCKED_BY_USER_ASSET_URLS | `plan:roll20-asset-relink` reports `RELINK_MAP_REQUIRED`: AW2E and YSHY are `MISSING_RELINK`. `plan:roll20-chat-browser-paint` reports `BROWSER_PAINT_BLOCKED_BY_RELINK`. Import and export dialogs can now generate commented relink-map drafts, and export smoke verifies the draft path. | The product can guide users toward the needed map, but it still needs real user-owned HTTP(S) replacement URLs before background/paint pixels can be judged or renderer CSS promoted for those fixtures. |
