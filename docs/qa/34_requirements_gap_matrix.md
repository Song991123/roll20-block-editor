...
| VERIFY | P1 | Absolute positioning inside frames/groups. | Drag commit supports containing block measurement and relative parent fallback. `scripts/edit_flow_browser_smoke.mjs` now verifies a synthetic frame child drag: the frame receives a design CSS `position: relative` rule, the child receives a design CSS `position: absolute; left/top` rule, and emitted left/top matches computed left/top in the Shadow edit render. | Add explicit user-facing affordance/mode for choosing flow vs absolute inside a frame, then expand coverage to imported real sheets. |
