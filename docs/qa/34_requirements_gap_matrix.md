...
`customcharsheet_json`. Roll20 rendered all 3 generated fixtures in the
character iframe. Latest screenshot diff reports AW2E sandbox mismatch `14.90%`
from viewport evidence, Les-Oublies sandbox mismatch `6.57%` from
DPR-corrected full-root evidence, and YSHY sandbox mismatch `45.05%` from
viewport evidence. Latest status is still partial:
...
| DOING | P0 | Actual Roll20 visual comparison. | Local baseline and pre-upload gate pass for the latest state-map run: `corepack pnpm run verify:roll20-preupload -- reports\roll20-actual-compare\2026-06-18-state-map-v1 --fixtures test-fixtures\visual --out-dir ./out --base-path /roll20-block-editor --state-map reports\visual-state-candidates\visual-state-candidates-state-map.json`. Chrome reached the dedicated Custom Sheet Sandbox. The file chooser path remains unreliable, but the endpoint fallback applied Les-Oublies, AW2E, and YSHY by POSTing base64 HTML/CSS/translation to `/sheetsandbox/savesheetsettings` and saving each fixture's `customcharsheet_json` through `/campaigns/savesettings/21639681`. Roll20 rendered all 3 generated sheets in the character iframe. Latest screenshot diff reports AW2E sandbox mismatch `14.90%` from viewport evidence, Les-Oublies sandbox mismatch `6.57%` from DPR-corrected full-root evidence, and YSHY sandbox mismatch `45.05%` from viewport evidence. Classifier records AW2E/YSHY as `viewport/crop/sheet size dominates current diff` and Les-Oublies as `sheet root geometry/height differs after full-height capture`. Status reports `PARTIAL_GENERATED_ACTUAL_SCREENSHOTS`, `generatedActualScreenshots=3/6`, `generatedDiffed=3/6`, `roomObservationScreenshots=0/3`, and `roomObservationDiffed=0/3`; `--require-actual` still fails because all 3 Roll20 chat screenshots are missing. This is broader partial actual evidence, not visual parity. | Capture normalized DPR-corrected sheet-root/full-root evidence for AW2E/YSHY, add trustworthy Roll20 chat screenshot or DOM-to-screenshot evidence, then compare actual vs local Sandbox expected geometry before production renderer CSS changes. Room View Check remains observation-only. |
