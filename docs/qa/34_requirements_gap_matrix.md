...
| PARTIAL | P0 | Universal HTML mapping. | Common semantic, form, table, list, conditional, repeating, Rolltemplate, and raw fallback paths have synthetic tests. | Expand uncommon and malformed structure coverage without source-specific branches. |
| PARTIAL | P0 | Universal CSS mapping. | Common declarations, complex selector fallback, at-rules, managed CSS, and separate compatibility transforms have tests. | Expand font, asset, custom-property, animation, and unsupported syntax coverage. |
| PARTIAL | P0 | Translation and default state. | Translation import/export and generic checked/default visibility paths work locally. | Verify the same current generated payload in both Roll20 destinations. |
