...
| VERIFY | P1 | Before/after/inside drop zones. | Layer panel row dragover now separates top/middle/bottom into `before`, `inside`, and `after`; adapter has top-level `moveBlockBefore`/`moveBlockAfter`, `inside` uses container nesting, and leaf children inside a Blockly statement chain can reorder before/after siblings. `scripts/edit_flow_browser_smoke.mjs` verifies row modes plus nested input reorder in emitted HTML. | Add non-leaf subtree reordering where safe, imported-sheet layer reorder coverage, and canvas insertion indicators. |
