...
| PARTIAL | P0 | Sheet Worker support. | A separate Worker workspace and useful parser/runtime subset exist. Unsupported statements remain lossless raw code and now expose source-derived import/inspector reasons without entering emitted output. Blockly selection opens the matching inspector. Ordinary scripts remain in a separate inert workspace and leave Roll20 `sheet.html` with a ZIP text backup. | Expand structured Worker syntax and runtime coverage; current diagnostics do not make raw syntax editable. |
