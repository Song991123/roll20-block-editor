...
| VERIFY | P1 | Before/after/inside drop zones. | Layer panel row dragover now separates top/middle/bottom into `before`, `inside`, and `after`; adapter has top-level `moveBlockBefore` and `moveBlockAfter`, while `inside` uses container nesting. `scripts/edit_flow_browser_smoke.mjs` verifies the row modes are `before,inside,after`. | Add nested sibling before/after support where Blockly statement chains allow it, plus canvas insertion indicators. |
