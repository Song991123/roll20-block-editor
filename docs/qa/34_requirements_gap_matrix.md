...
| PARTIAL | P0 | Sheet Worker support. | A separate Worker workspace and useful parser/runtime subset exist. Unsupported Worker statements remain raw; ordinary scripts use a separate inert source workspace, stay hidden, and are excluded from Roll20 `sheet.html` with a ZIP text backup. | Expand unsupported Worker syntax coverage and diagnostics. |
