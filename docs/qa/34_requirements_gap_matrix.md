...
| PARTIAL | P0 | Universal HTML mapping. | Common semantic, form, table, list, conditional, repeating, Rolltemplate, raw fallback, and inert Page JS paths have synthetic tests; the combined browser fixture checks current default-state structure. | Expand uncommon and malformed structure coverage without source-specific branches. |
| PARTIAL | P0 | Universal CSS mapping. | Common declarations, complex selector fallback, managed CSS, custom properties, nested conditional at-rules, and separate compatibility transforms have tests. | Expand font, user-owned asset, animation, malformed selector, and unsupported syntax coverage. |
| PARTIAL | P0 | Translation and default state. | Translation, checked/selected defaults, visible/hidden conditional areas, and Preview/Edit parity pass in the current anonymous browser fixture. | Verify the same current generated payload in both Roll20 destinations. |
