...
| PARTIAL | P0 | Sheet Worker support. | A separate Worker workspace and useful parser/runtime subset exist. Canonical multi-event listeners, callback event variables, previous/new values, player/worker sources, and `setAttrs` silent/completion behavior pass local browser proof. Repeating templates now produce runtime rows; full, section-field, section-wide, and plain-field aliases plus Worker/player removal data pass local browser proof. Unsupported statements remain lossless raw code with source-derived diagnostics. Cold-load import waits for all model workspaces instead of reporting analysis without applying blocks. | Add repeating-row reorder and `_reporder` behavior, row-context shorthand, broader APIs, and actual Roll20 execution proof; arbitrary non-byte-stable formatting and unsupported statements still remain raw. |
