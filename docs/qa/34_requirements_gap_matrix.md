...
| PARTIAL | P0 | Universal HTML mapping. | Common semantic, form, table, list, conditional, repeating, Rolltemplate, raw fallback, and inert Page JS paths have synthetic tests; grouped radios, multi-select, readonly, disabled, and current default-state structure pass the combined browser path. | Expand remaining uncommon and malformed structure coverage without source-specific branches. |
