...
- `DONE LOCAL`: common worker `Math.floor`, `Math.ceil`, `Math.round`,
  `Math.abs`, `Math.min`, `Math.max`, and numeric `parseInt` calls now map to
  editable reporter blocks when their supported argument shape is present.
...
fixture-A exposed a real verification-payload bug. The reference fixture-A `sheet.json`
...
iframe; applying the reference fixture-A source HTML/CSS/translation plus original
...
`1.75%`, fixture-B `2.02%`, fixtureC `1.01%`. Remaining actual-Roll20 evidence
