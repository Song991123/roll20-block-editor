...
| TODO | P0 | Actual Roll20 visual comparison. | Roll20 authenticated/manual comparison not recorded in current report. Chrome session has Roll20 tabs available, but no room/sandbox comparison has been captured yet. | Run Room View Check for solo rooms and Custom Sheet Upload Check in sandbox/test room; store evidence in ignored `reports/roll20-actual-compare/`. |
