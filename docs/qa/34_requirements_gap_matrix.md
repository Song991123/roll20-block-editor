...
| PARTIAL | P0 | Sheet Worker support. | A separate Worker workspace and useful parser/runtime subset exist. Canonical multi-event listeners, callback event variables, previous/new values, player/worker sources, and `setAttrs` silent/completion behavior pass local browser proof. Unsupported statements remain lossless raw code with source-derived diagnostics. Cold-load import now waits for all model workspaces instead of reporting analysis without applying blocks. | Expand repeating-section aliases, removal data, and broader APIs; arbitrary non-byte-stable formatting and unsupported statements still remain raw. |
