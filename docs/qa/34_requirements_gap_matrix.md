...
| PARTIAL | P0 | Figma-like direct manipulation. | Flow/free placement, before/inside/after, grouping, container roles, multi-move, long-list navigation, and nested 2D individual translate/rotate/scale plus transform-list stacks work locally. Same-parent first free placement uses a guarded node-attribute patch, keeps runtime class/style state, and falls back on any unrelated HTML change in modern and legacy browser smoke. | Reduce the remaining parent apply-scheduling/ACK warning on larger ignored input; review dense imported trees, longer/dynamic or animated transform stacks, 3D/perspective, and longer mixed histories. |
...
| PARTIAL | P0 | Sheet Worker support. | A separate Worker workspace and useful parser/runtime subset exist. Canonical multi-event listeners, callback event variables, previous/new values, player/worker sources, and `setAttrs` silent/completion behavior pass local browser proof. Repeating templates produce shared runtime rows across duplicate same-name fieldsets; full, section-field, section-wide, and plain-field aliases, row-context shorthand, synchronized reorder/`_reporder`, and Worker/player removal data pass local browser proof. Unsupported statements remain lossless raw code with source-derived diagnostics. Cold-load import waits for all model workspaces instead of reporting analysis without applying blocks. | Add broader APIs and actual Roll20 execution proof; arbitrary non-byte-stable formatting and unsupported statements still remain raw. |
