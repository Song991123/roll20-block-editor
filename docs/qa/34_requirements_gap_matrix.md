...
| PARTIAL | P0 | Sheet Worker support. | A separate Worker workspace and useful parser/runtime subset exist. Canonical multi-event listeners, callback event variables, and common event fields are structurally editable; local browser proof covers both registered events plus player/worker source types. Unsupported statements remain lossless raw code with source-derived diagnostics. Ordinary scripts remain inert and leave Roll20 `sheet.html` with a ZIP text backup. | Expand structured syntax and runtime event fields; arbitrary non-byte-stable formatting and unsupported statements still remain raw. |
