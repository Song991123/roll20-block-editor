...
| VERIFY | P1 | Rolltemplate/chat rendering. | `scripts/rolltemplate_chat_smoke.mjs` verifies local preview iframe roll button -> ChatPane card rendering for 3 ignored fixtures, clears chat per fixture, and checks 1 card, 280px rolltemplate width, and no app-only `rolltemplate:name` debug label. AW2E/YSHY passed with real user-click; Les-Oublies passed only through `dom-click-fallback`, which means runtime rendering works but default visible-state/actionability still needs work. | Add actual Roll20 sandbox/test-room chat smoke, improve Les-Oublies default-state/actionability to user-click, and compare Roll20 chat pane message styling against a real room/sandbox capture. |
