...
| VERIFY | Codex | Split requirements into actionable gap matrix and branch plan. | Added `docs/qa/34_requirements_gap_matrix.md`, `docs/operations/34_branch_and_deployment_plan.md`, and CI workflow. Needs lint/build, commit, push, and `dev` branch push. |
...
| VERIFY | P0 | Establish `dev` branch and predeploy CI. | New CI workflow runs lint/build on `main`, `dev`, and PRs. `main` remains the only GitHub Pages deploy branch. |
