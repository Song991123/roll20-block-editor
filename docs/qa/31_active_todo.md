...
`customcharsheet_json`. Roll20 rendered both generated sheets in the character
iframe. Ignored viewport screenshots now exist for all 3 prepared fixtures:
AW2E sandbox mismatch `14.90%`, Les-Oublies DPR-corrected full-root mismatch
`6.57%`, and YSHY viewport mismatch `45.05%`. Latest status is still partial:
...
| DOING | Codex | Add Roll20 actual-screen verification workflow. | Latest local pre-upload gate PASS with state-map-aware baseline: `corepack pnpm run verify:roll20-preupload -- reports\roll20-actual-compare\2026-06-18-state-map-v1 --fixtures test-fixtures\visual --out-dir ./out --base-path /roll20-block-editor --state-map reports\visual-state-candidates\visual-state-candidates-state-map.json`. Chrome file chooser upload remains unreliable, but the dedicated sandbox endpoint fallback now applied Les-Oublies, AW2E, and YSHY by POSTing base64 HTML/CSS/translation to `/sheetsandbox/savesheetsettings` and saving each fixture's `customcharsheet_json` through `/campaigns/savesettings/21639681`. Roll20 rendered all 3 generated sheets in the character iframe. Latest `node scripts\roll20_actual_screenshot_diff.mjs reports\roll20-actual-compare\2026-06-18-state-map-v1` reports sandbox mismatches: AW2E `14.90%` from viewport evidence, Les-Oublies `6.57%` from DPR-corrected full-root evidence, and YSHY `45.05%` from viewport evidence. Latest classifier PASS records AW2E/YSHY as `viewport/crop/sheet size dominates current diff` and Les-Oublies as `sheet root geometry/height differs after full-height capture`. Latest status command reports `PARTIAL_GENERATED_ACTUAL_SCREENSHOTS`, `generatedActualScreenshots=3/6`, `generatedDiffed=3/6`, `roomObservationScreenshots=0/3`, and `roomObservationDiffed=0/3`; `--require-actual` still correctly fails because all 3 Roll20 chat screenshots are missing. A Roll20 roll button click previously created one Les-Oublies `sheet-rolltemplate-classic-roll` chat DOM message, but chat screenshot capture was not trustworthy and remains missing. This is broader partial actual Roll20 evidence, not visual parity. Next: capture DPR-corrected sheet-root/full-root evidence for AW2E/YSHY and a trustworthy Roll20 chat screenshot path before renderer CSS promotion. |
...
| DOING | P0 | Run Roll20 actual-screen check with Chrome session. | Clean local payloads are generated by `scripts/roll20_actual_local_baseline.mjs` and gated by `corepack pnpm run verify:roll20-preupload -- reports\roll20-actual-compare\2026-06-18-state-map-v1 --fixtures test-fixtures\visual --out-dir ./out --base-path /roll20-block-editor --state-map reports\visual-state-candidates\visual-state-candidates-state-map.json`, latest PASS. Chrome reached the dedicated Roll20 Custom Sheet Sandbox. File chooser upload remained blocked, but direct sandbox endpoint application now succeeded for Les-Oublies, AW2E, and YSHY: POST `/sheetsandbox/savesheetsettings` accepted generated HTML/CSS/translation, `customcharsheet_json` was saved on the settings page, and a sandbox character opened the generated sheet iframe. Latest actual screenshot diff reports AW2E sandbox mismatch `14.90%`, Les-Oublies sandbox mismatch `6.57%` using DPR-corrected full-root evidence, and YSHY sandbox mismatch `45.05%`. Latest classifier records AW2E/YSHY as viewport/crop/sheet-size dominated, so those screenshots are not enough for renderer CSS conclusions. Status now reports `PARTIAL_GENERATED_ACTUAL_SCREENSHOTS`, `generatedActualScreenshots=3/6`, `generatedDiffed=3/6`, `roomObservationScreenshots=0/3`, and `roomObservationDiffed=0/3`; `--require-actual` still fails because all Roll20 chat screenshots are missing. Existing solo rooms are observation-only; generated sheet checks must use Custom Sheet Sandbox first or a new test room. Store screenshots/reports locally only. |
