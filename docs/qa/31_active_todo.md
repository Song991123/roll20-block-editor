...
| DONE | Codex | Add imported fixture preview/edit screenshot smoke. | `scripts/preview_edit_visual_smoke.mjs` PASS after `build`: 3 fixtures rendered preview root + edit root with 0 console/page errors. Latest diagnostic mismatch after removing persistent edit-only drop outlines: AW2E 25.51%, Les-Oublies 0% (10 pixels), YSHY 1BU 4.03%; this is not Roll20 parity yet. |
...
| --- | ---: | --- | --- |
| VERIFY | P0 | Make edit canvas and preview render from the same emitted HTML/CSS path, with edit overlays only. | `scripts/preview_edit_visual_smoke.mjs` now captures imported fixture preview root vs edit root with coarse mismatch bounds/quadrants. Pipeline PASS after removing persistent edit-only drop outlines. Remaining diagnostic mismatch: AW2E 25.51% (bounds 0,10 850x1070), Les-Oublies 0%/10 px, YSHY 1BU 4.03% (bounds 0,223 850x857). Needs fixture-specific difference fixes before DONE. |
