...
| VERIFY | P1 | Expand Roll20 worker simulator and chat rolltemplate rendering. | Local chat smoke added: `scripts/rolltemplate_chat_smoke.mjs` PASS for 3 ignored fixtures through preview iframe -> ChatPane (`reports/rolltemplate-chat-smoke/rolltemplate-chat-smoke-results.md`). AW2E/YSHY used real `user-click`; Les-Oublies required `dom-click-fallback`, so default visible-state/actionability remains TODO. Actual Roll20 chat parity remains TODO. Worker simulator split still TODO. |
...
| VERIFY | Local rolltemplate chat smoke | `scripts/rolltemplate_chat_smoke.mjs` PASS for AW2E, Les-Oublies, and YSHY 1BU in the static app. This proves local preview roll button -> ChatPane card rendering only; Les-Oublies used `dom-click-fallback`, and actual Roll20 chat/sandbox parity is still unverified. |
