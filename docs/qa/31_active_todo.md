...
- BLOCKED VISUAL EVIDENCE: r20 correctly FAILed because all `11` images failed in each local mode and both direct/proxy font requests failed. Resulting local roots (`1187x1879` modern, `895x1861` legacy) are asset-incomplete and must not be compared as pixel/geometry parity against actual Roll20.
- CURRENT P0: Obtain an asset-complete same-browser local run, then compare modern and legacy root/table geometry independently. Keep user-owned relink, network reachability, runtime asset transformation, and renderer CSS as separate causes; do not add fixture-specific compensation.
