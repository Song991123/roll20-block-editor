...
| DONE | Codex | Add imported real-fixture edit drag sync smoke. | `scripts/imported_edit_sync_smoke.mjs` PASS after `build`: AW2E, Les-Oublies, and YSHY 1BU each found an imported visible node that moved through the real edit pointer path, landed at the same position in preview, and emitted matching absolute CSS. Report: ignored `reports/imported-edit-sync/imported-edit-sync-results.md`. This is local static-app evidence only, not actual Roll20 parity. |
...
| --- | ---: | --- | --- |
| VERIFY | P0 | Make edit canvas and preview render from the same emitted HTML/CSS path, with edit overlays only. | `scripts/preview_edit_visual_smoke.mjs` PASS after this slice: AW2E 4.96%, Les-Oublies 4.76%, YSHY 1BU 1.25%. `scripts/imported_edit_sync_smoke.mjs` also PASS for 3 imported fixtures after fixing Shadow image referrer behavior and optimistic move clearing. Needs remaining fixture-specific visual fixes and actual Roll20 comparison before DONE. |
...
| DONE | Imported real-fixture edit sync smoke | `scripts/imported_edit_sync_smoke.mjs` PASS for AW2E, Les-Oublies, and YSHY 1BU against static `out/`: each selected imported node moved through the real edit pointer path, preview landed on the same block position, and emitted HTML/CSS carried matching absolute position data. Shadow edit image loading was aligned with iframe preview via `referrerPolicy="no-referrer"` for sheet images. Local evidence only; actual Roll20 parity remains unverified. |
