...
| VERIFY | P1 | Make layer panel useful as a Figma-like hierarchy/reparenting surface. | Layer rows expose explicit drag zones (`before`, `inside`, `after`), adapter supports top-level `moveBlockAfter`, and leaf children inside a statement chain can be reordered before/after siblings. `edit_flow_browser_smoke` verifies zone detection and nested input reorder in emitted HTML. Still needs non-leaf subtree reordering, visible insertion line screenshots during real user drag, and broader imported-sheet coverage. |
