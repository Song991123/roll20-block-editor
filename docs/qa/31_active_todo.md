...
Pre-upload/evidence guard caveat: rerunning `corepack pnpm run
verify:roll20-preupload -- reports\roll20-actual-compare\2026-06-18-state-map-v1
--fixtures test-fixtures\visual --out-dir ./out --base-path
/roll20-block-editor --state-map
reports\visual-state-candidates\visual-state-candidates-state-map.json` currently
FAILs at `payload-roundtrip` because AW2E/YSHY remain above the strict local
2% roundtrip gate; Les-Oublies remains the only fixture with generated actual
Roll20 full-root screenshot evidence in this run.
