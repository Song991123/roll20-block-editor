...
| TODO | P0 | Run Roll20 actual-screen check with Chrome session. | Local baseline payloads are now generated by `scripts/roll20_actual_local_baseline.mjs` for 3 ignored fixtures with no blocking export warnings. `scripts/roll20_actual_screenshot_diff.mjs` is ready to compare `roll20-sandbox.png`, `roll20-room.png`, and `roll20-chat.png` when actual captures exist; latest run correctly leaves all actual targets SKIP. Existing solo rooms are observation-only; generated sheet checks must use Custom Sheet Sandbox first or a new test room. Store screenshots/reports locally only. |
