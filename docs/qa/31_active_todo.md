...
| VERIFY | Local rolltemplate chat smoke | `scripts/rolltemplate_chat_smoke.mjs` reports AW2E/YSHY PASS and Les-Oublies SKIP in the static app. Each tested fixture clears prior chat first and verifies 1 card, 280px width, `Debug label=no`. Les-Oublies has 40 roll buttons in DOM but 0 visible/actionable buttons in the default render, so chat rendering cannot be honestly tested from its default screen yet. Actual Roll20 chat/sandbox parity is still unverified. |
