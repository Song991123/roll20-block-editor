...
| DONE | Codex | Add imported real-fixture edit drag sync smoke. | `scripts/imported_edit_sync_smoke.mjs` PASS after `build`: AW2E, Les-Oublies, and YSHY 1BU each found an imported visible input node that moved through the real edit pointer path, landed at the same position in preview, emitted matching absolute CSS, accepted a friendly widget drop into a visible imported sheet insertion target as non-absolute flow content, and survived edited emit -> re-import -> emit stability checks. Report: ignored `reports/imported-edit-sync/imported-edit-sync-results.md`. Les-Oublies also found a safe imported layer leaf sibling pair and reordered it through the layer row path; AW2E/YSHY had no safe imported leaf pair and record SKIP for that sub-check. Resource diagnostics still classify remaining edit/reimport asset failures, currently YSHY 38. No page errors. This is local static-app evidence only, not actual Roll20 parity. |
| DOING | Codex | Add Roll20 actual-screen verification workflow. | Latest local baseline package PASS: `reports/roll20-actual-compare/2026-06-18-actual-diff-ready/local-baseline-results.md` generated preview/edit screenshots plus Sandbox payload files and `upload.zip` for 3 ignored fixtures. Roll20 Custom Sheet Sandbox was reached in Chrome and a new isolated verification sandbox was created; `Sheet Sandbox Tools` exposed HTML/CSS/Translation file inputs. Upload attempt is currently blocked by Chrome extension file access (`fileChooser.setFiles failed: Not allowed`). Local evidence: ignored `reports/roll20-actual-compare/2026-06-18-actual-diff-ready/roll20-sandbox-observation/`. Next check: enable file URL access for the Codex extension, upload payload, capture `roll20-sandbox.png`, rerun screenshot diff helper. |
...
| VERIFY | P1 | Make layer panel useful as a Figma-like hierarchy/reparenting surface. | Layer rows expose explicit drag zones (`before`, `inside`, `after`), adapter supports top-level and nested leaf sibling insertion, and leaf children inside a statement chain can be reordered before/after siblings. Canvas widget dragover now exposes `inside`, `before`, and `after`; `edit_flow_browser_smoke` verifies zone detection, nested input reorder, and canvas insertion of new inputs before and after an existing nested input in emitted HTML. `imported_edit_sync_smoke` now also verifies imported canvas insertion as non-absolute flow content for 3 fixtures and imported layer reorder for Les-Oublies when a safe leaf sibling pair exists. Still needs non-leaf subtree reordering, broader imported-sheet coverage, and richer screenshot evidence for real user drags. |
...
| TODO | P0 | Run Roll20 actual-screen check with Chrome session. | Local baseline payloads are generated by `scripts/roll20_actual_local_baseline.mjs` for 3 ignored fixtures with no blocking export warnings. Chrome reached Roll20 Custom Sheet Sandbox and created an isolated verification sandbox; upload is blocked until Chrome allows local file access for the Codex extension. `scripts/roll20_actual_screenshot_diff.mjs` is ready to compare `roll20-sandbox.png`, `roll20-room.png`, and `roll20-chat.png` when actual captures exist. Existing solo rooms are observation-only; generated sheet checks must use Custom Sheet Sandbox first or a new test room. Store screenshots/reports locally only. |
...
| DONE | Edit-flow browser smoke (real DragEvents) | `scripts/edit_flow_browser_smoke.mjs` PASS in headless Chromium against static `out/`: hook flow/absolute paths + real dragover/drop nesting into a frame container, canvas drop indicator state (`hostDropMode=inside`, active target mode `inside`), canvas sibling insertion indicators (`before` and `after`), canvas insertion of new inputs before and after an existing nested input in emitted HTML, existing-object drag, layer row drop-zone detection (`before,inside,after`), and nested input reorder reflected in emitted HTML. Report: `reports/edit-flow-smoke/edit-flow-smoke-results.md`. `lint`/`build` re-passed on 2026-06-18. |
| DONE | Imported real-fixture edit sync smoke | `scripts/imported_edit_sync_smoke.mjs` PASS for AW2E, Les-Oublies, and YSHY 1BU against static `out/`: each selected imported input moved through the real edit pointer path, preview landed on the same block position, emitted HTML/CSS carried matching absolute position data, a friendly widget inserted into an imported sheet target as non-absolute flow content, and the edited emit survived a re-import/emit cycle. Les-Oublies additionally verified imported layer leaf reorder; AW2E/YSHY record SKIP for that sub-check because no safe imported leaf sibling pair was found. Local evidence only; actual Roll20 parity remains unverified. |
