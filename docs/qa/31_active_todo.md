...
| DONE | Codex | Add imported fixture preview/edit screenshot smoke. | `scripts/preview_edit_visual_smoke.mjs` PASS after `build`: 3 fixtures rendered preview root + edit root with 0 console/page errors. Diagnostic mismatch remains: AW2E 29.95%, Les-Oublies 24.59%, YSHY 1BU 8.07%; this is not Roll20 parity yet. |
| DOING | Codex | Add Roll20 actual-screen verification workflow. | Chrome can reach logged-in Roll20 `campaigns/search` page. Next check: local baseline -> solo-room observation -> Custom Sheet Sandbox/test-room upload check. Evidence must stay in ignored `reports/roll20-actual-compare/`. |

## Critical Product Tasks

| Status | Priority | Task | Notes |
| --- | ---: | --- | --- |
| VERIFY | P0 | Make edit canvas and preview render from the same emitted HTML/CSS path, with edit overlays only. | `scripts/preview_edit_visual_smoke.mjs` now captures imported fixture preview root vs edit root. Pipeline PASS, but diagnostic mismatch remains: AW2E 29.95%, Les-Oublies 24.59%, YSHY 1BU 8.07%. Needs difference classification/fixes before DONE. |
