...
- Added ChatPane translation application for rolltemplate field text and simple `data-i18n` labels via the existing Roll20 translation normalizer. Local YSHY chat now renders Korean labels such as `Í∑ºÎ†•`, `Í∏∞Ï§ÄÏπò`, and `Íµ¥Î¶º`.
...
- Folder guide exists at the parent level as `?¥Îçî ?àÎÇ¥.md`; update it only when actual top-level folder roles change.
...
- Added a user-facing edit placement mode control with `?êÎ¶Ñ` and `?êÏú†` choices. `?êÎ¶Ñ` keeps gallery drops as flow/nesting operations; `?êÏú†` lets a drop inside a capable frame become a nested absolute child.
...
- Expanded `scripts/edit_flow_browser_smoke.mjs` with a copyright-safe synthetic free-placement import. The smoke clicks `?êÏú†`, drops a gallery text input into a frame, and verifies the emitted HTML nests the input inside the frame with matching computed/emitted absolute coordinates.
- Latest local ignored validation: `corepack pnpm run lint` PASS, `corepack pnpm run build` PASS, and `scripts/edit_flow_browser_smoke.mjs` PASS.
...
- Added the toolbar `Íµ¨Î≤Ñ??CSS` toggle (`data-testid="preview-legacy-css-toggle"`) so users can compare modern/original CSS preview against legacy Roll20 CSS sanitize locally before export.
...
- Added a user-facing `Roll20 ?ÖÎ°ú??Ï§ÄÎπ??ÅÌÉú` section to `components/editor/ExportDialog.tsx`.
...
- Latest static-app validation: `corepack pnpm run smoke:export-dialog -- --out-dir ./out --base-path /roll20-block-editor --report-dir reports/export-dialog-smoke` PASS. It opens the export dialog, confirms 5 readiness items, confirms the `?§Ï†ú Í≤ÄÏ¶??ÑÏöî` badge, opens the import dialog, and verifies main mode tab clicks with 0 console/page errors.
...
- Cleaned the edit canvas and layer panel user-facing copy:
  - edit toolbar labels now use readable `?úÌä∏ ?∏Ïßë`, `?êÎ¶Ñ`, `?êÏú†`, and Korean placement tooltips.
  - layer role labels now use Korean names such as `?Ä`, `?êÎ¶Ñ`, `??, `?ÖÎ†•`, and `Î≤ÑÌäº`.
  - friendly widget gallery preset names/descriptions now use readable Korean copy.
- Hardened `scripts/edit_flow_browser_smoke.mjs` with an `editUiCopy` check. It now verifies the edit canvas contains `?úÌä∏ ?∏Ïßë`, `?àÏù¥??, `?àÏù¥??Í≤Ä??, `?êÎ¶Ñ`, and `?êÏú†`, and that the scoped edit canvas text has no Han-range mojibake.
- Latest validation:
...
- Hardened `scripts/export_dialog_browser_smoke.mjs` so it now verifies:
  ...
  - export dialog title, 5 readiness items, `?§Ï†ú Í≤ÄÏ¶??ÑÏöî` badge, legacy toggle copy, and local-vs-actual Roll20 verification warning;
...
- While actual Roll20 upload is blocked, improved edit-layer row affordance:
  ...
  - rows visibly show the role label, `?¥Í∏∞ Í∞Ä?? for containers, and default placement mode (`?êÎ¶Ñ` / `?êÏú†`);
  - drag target badges now use Korean labels (`?ûÏóê ?£Ïùå`, `?àÏóê ?£Ïùå`, `?§Ïóê ?£Ïùå`) instead of raw `before/inside/after`.
...
- Validation:
  ...
  - `corepack pnpm run lint`: PASS.
  - `corepack pnpm run build`: PASS.
  - `corepack pnpm run smoke:edit-flow -- --out-dir ./out --base-path /roll20-block-editor --report-dir reports/edit-flow-smoke --port 4318`: PASS; the section layer row reported `roleKind=frame`, `canDrop=1`, `defaultDropMode=flow`, and visible text including `?Ä`, `?¥Í∏∞ Í∞Ä??, and `?êÎ¶Ñ`.
...
- Added a user-facing `Roll20 Sandbox ?àÏÉÅ Î≥Ä?? panel to `components/editor/ExportDialog.tsx`.
...
- Latest imported-fixture validation:
  ...
  - Export dialog reported `ÏπòÎ™Ö ?§Î•ò ?ÜÏùå`, 4 diagnostics rows, expected rewrite rows for HTML/CSS/classes, 0 console/page errors, and no mojibake.
...
- Added `roll20SandboxSanitize` to `usePreviewStore`, exposed it through `window.__perfHook.setRoll20SandboxSanitize()`, and surfaced a compact `Sandbox ?àÏÉÅ` toggle in the main toolbar whenever preview is visible.
...
- Reopened the sandbox character in the Roll20 editor. AW2E and YSHY both rendered in the character iframe; YSHY was additionally confirmed by iframe text markers such as `?¥Î¶Ñ`, `?åÎ†à?¥Ïñ¥`, `ÏßÅÏóÖ`, `?òÏù¥`, `?¥ÏÑ±`, and `Í∑ºÎ†•`.
