...
- `corepack pnpm run verify:roll20-preupload -- reports\roll20-actual-compare\2026-06-18-state-map-v1 --fixtures test-fixtures\visual --out-dir ./out --base-path /roll20-block-editor --state-map reports\visual-state-candidates\visual-state-candidates-state-map.json` currently FAILs at `payload-roundtrip`; the existing all-fixture roundtrip report still has AW2E/YSHY above the strict 2% local gate while Les-Oublies passes. This means the evidence guard also remains non-green until that payload roundtrip gap is fixed or the guard is scoped to a verified single fixture.
