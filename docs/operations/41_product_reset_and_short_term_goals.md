...
| Legacy Roll20 runtime | 2/4 | A newly created dedicated room with exactly one visible member saved the same anonymous payload with Legacy sanitization enabled; translation, authored CSS, input/Roll, and chat output rendered. An initial 420x180 crop measured 5.913% threshold-20 mismatch, so parity is not claimed. | Context-aware crop/computed-style sidecars, live worker proof, and broader sheet coverage. |
