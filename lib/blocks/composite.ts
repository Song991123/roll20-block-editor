...
export const COMPOSITE_BLOCKS: BlockDef[] = [
  ...
  //
  // Stage 22 §4 — `r20_value_switch_panel` (legacy-sheet-corpus era 토글 같은 일반화 패턴).
  ...
  // CASES 비면 wrapper 만 emit (warning).
  {
    ...
    label: '묶음: 값별 영역 전환',
    tooltip:
      '값에 따라 영역 toggle — name="attr_NAME" 라디오 + sibling CSS 자동 emit (legacy-sheet-corpus era 패턴 일반화).',
    init: mkInit((b) => {
