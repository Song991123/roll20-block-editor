...
export interface PerfHook {
  ...
  /**
   * 합성 (synthetic) Blockly XML 생성기 — legacy-sheet-corpus 식별자 / 사용자 시트 토큰 0.
...
function buildHook(): PerfHook {
  ...
  return {
    ...
    genSyntheticXml: (n: number, prefix: string = 'syn'): string => {
      ...
      // legacy-sheet-corpus legacy corpus 실 import 도 top-level 다수 + 얕은 nesting 이므로 flat 구조가
      //   더 충실한 시뮬레이션.
      // 시트 specific 0: NAME 필드 = `${prefix}${i}` (legacy-sheet-corpus 식별자 / 한글 0).
