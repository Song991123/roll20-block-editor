...
function classifyRawStatement(statement: string): RawWorkerDiagnosticCode {
  ...
  if (/^(?:on|getAttrs|setAttrs|getSectionIDs|setSectionOrder|generateRowID|removeRepeatingRow|getTranslationByKey|getTranslationByLang|getTranslationLanguage|getCompendiumPage|getCompendiumEntries|startRoll|finishRoll)\s*\(/.test(code)) {
