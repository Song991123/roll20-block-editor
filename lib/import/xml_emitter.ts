 ...
 *   상 top-level 로 N 개 그대로 박힘. (이게 legacy-sheet-corpus legacy corpus import 의 의도된 동작.)
...
export function emitWorkspaceXml(top: MatchedBlock[]): string {
  ...
  // (`.toggle:checked ~ div.target`) 가 깨짐. legacy-sheet-corpus legacy corpus era toggle 등.
  const Y_STEP = 40;
 ...
 * 상 작음 (legacy-sheet-corpus legacy corpus 최대 depth ~ 10). 사용자 시트 specific 가정 0.
 */
