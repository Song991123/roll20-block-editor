...
| `corepack pnpm run diff:visual-fixtures` | Generate visual diff pages, run them in headless Chromium, collect `visual-fixture-diff-results.md/.json`, and write a heuristic cause classification report with state selector/default input/dimension clues. This automates reference-image diagnostics but is still not a Roll20 visual parity gate. |
...
| `corepack pnpm run smoke:visual-state-candidates -- --out-dir ./out --base-path /roll20-block-editor --fixtures test-fixtures/visual --report-dir reports/visual-state-candidates` | Import visual fixtures, capture initial plus visible `button[type=action]` state candidates, and compare each screenshot to the copied reference image. Local state/crop triage only; not actual Roll20 parity. Needs `corepack pnpm run build` first. |
...
| `corepack pnpm run smoke:worker-state -- --out-dir ./out --base-path /roll20-block-editor --report-dir reports/sheet-worker-state-smoke` | Browser smoke for Roll20-style action button -> worker `setAttrs` -> hidden input value attribute -> CSS `[value=...]` state selector updates. Local preview iframe only; not actual Roll20 parity. Needs `corepack pnpm run build` first. |
