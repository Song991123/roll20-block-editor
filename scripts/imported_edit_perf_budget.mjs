 ...
 * scripts/imported_edit_sync_smoke.mjs. Output contains anonymous fixture
 * labels, timings,
...
// Imported fixture identifiers and paths are never emitted, even when a
// caller passes a legacy `--redact-ids false` argument.
const REDACT_IDS = true;
...
function runSelfTest() {
  const privateId = 'private-fixture-name-must-not-leak';
  const fixture = summarizeFixture({
    id: privateId,
    import: { totalMs: 1, injectMs: 1, emitMs: 1, blockCount: 1 },
    interactionPass: true,
    pass: true,
    reimport: { stable: { html: true, css: true, i18n: true } },
  }, 0);
  const summary = {
    generatedAt: 'self-test',
    source: 'local-anonymous-results',
    status: fixture.status,
    budgets: BUDGETS,
    fixtures: [fixture],
  };
  const output = `${JSON.stringify(summary)}\n${renderMarkdown(summary)}`;
  if (fixture.id !== 'fixture-1' || fixture.sourceIdRedacted !== true || output.includes(privateId)) {
    throw new Error('privacy invariant failed: imported fixture identity escaped the report');
  }
  if (output.includes('imported-edit-sync-results.json') || output.includes('\\')) {
    throw new Error('privacy invariant failed: local source path escaped the report');
  }
  console.log('imported_edit_perf_budget privacy self-test: PASS');
}
